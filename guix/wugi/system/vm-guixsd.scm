  ...
  (program-file
   ...
   #~(begin
       ...
       (and
        ...
        (iptables "-I" "INPUT" "-p" "tcp" "--destination" #$(string-append %vm-guixsd-private-ip-address "/32") "--dport" "9091" "-j" "ACCEPT")
        ;; Accept DNS traffic, which is required for Docker containers.
        (iptables "-I" "INPUT" "-p" "tcp" "--destination" #$(string-append %vm-guixsd-private-ip-address "/32") "--dport" "53" "-j" "ACCEPT")
        (iptables "-I" "INPUT" "-p" "udp" "--destination" #$(string-append %vm-guixsd-private-ip-address "/32") "--dport" "53" "-j" "ACCEPT")
        ...
        ;; Forward connections from %vm-guixsd-private-ip-address:6443 to
        ...
        ;; Fault
        (iptables "-t" "nat" "-A" "PREROUTING" "-p" "tcp" #$(format #f "--destination ~a" %vm-guixsd-private-ip-address) "--dport" "6443" "-j" "DNAT" "--to-destination" "192.168.154.1:6443")
        (iptables "-t" "nat" "-A" "POSTROUTING" "-p" "tcp" "-d" "192.168.154.1" "--dport" "6443" "-j" "SNAT" "--to-source" #$%vm-guixsd-private-ip-address)
...
")))
          ...
          (proxy "prometheus.wugi.info" 9090 #:listen %vm-guixsd-private-ip-address)
     ...
     (list (shepherd-service
            ...
            (start #~(make-forkexec-constructor
                      (list #$(file-append dnsmasq "/sbin/dnsmasq")
                            ...
                            "--enable-tftp"
                            #$(string-append "--server=" %vm-guixsd-private-ip-address)
                            "--no-resolv"
...
")))
         ...
         (dovecot-service
          #:config (dovecot-configuration
                    (listen (list "127.0.0.1" %vm-guixsd-private-ip-address))
...
127.0.0.1 ecdsa-sha2-nistp256 AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBOnaDeOzwmrcrq1D8slYaeFozXZ0cpqNU0EvGmgnO29aiKkSD1ehbIV4vSxk3IDXz9ClMVPc1bTUTrYhEVHdCks="))))
         ...
         (service libvirt-service-type
                  (libvirt-configuration
                   (listen-addr %vm-guixsd-private-ip-address)
                   (listen-tcp? #t)
                   (auth-tcp "none")))
