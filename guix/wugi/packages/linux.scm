...
(and=> (and=> (false-if-exception (and (resolve-interface '(nonguix licenses))
                                       (resolve-interface '(gnu packages linux))))
              (lambda (module)
                (when (module-defined? module 'linux-libre-5.13)
...
System on hardware which requires nonfree software to function.")))
                     ...
                     linux-5.13-with-bpf)))))
       (lambda (package)
         (let ((variable (make-variable package)))
           (module-define! (current-module) 'linux-5.13-with-bpf variable)
           (module-export! (current-module) '(variable)))))

(and=> (and=> (false-if-exception (and (resolve-interface '(nongnu packages linux))
                                       (resolve-interface '(gnu packages linux))))
              (lambda (module)
                (when (module-defined? module 'linux-libre-5.15)
                  (save-module-excursion
                   (lambda ()
                     (set-current-module (make-user-module '()))
       ...
       (lambda (package)
         (let ((variable (make-variable package)))
           (module-define! (current-module) 'linux-5.15-with-bpf variable)
           (module-export! (current-module) '(variable)))))

(and=> (and=> (false-if-exception (resolve-interface '((nonguix licenses))))
              (lambda (_)
                (when (module-defined? module 'nonfree)
                  (save-module-excursion
                   (lambda ()
                     (set-current-module (make-user-module '()))
...
if your hardware is supported by one of the smaller firmware packages.")
       ...
       (lambda (package)
         (let ((variable (make-variable package)))
           (module-define! (current-module) 'linux-firmware variable)
           (module-export! (current-module) '(variable)))))
