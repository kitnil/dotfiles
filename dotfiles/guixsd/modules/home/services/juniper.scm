     ...
     #~(begin
         ...
         (let* ((directory
           ...
           (call-with-output-file (string-append directory "/bgp-neighbors.txt")
             (lambda (port)
               (display "group WEBA\n" port)
               (display #$(juniper-command host '("cli" "show" "bgp" "neighbor" "10.102.0.1"))
                        port)
           ...
           (call-with-output-file (string-append directory "/route-advertising-protocol.txt")
             (lambda (port)
               (display "group PROMETEY\n" port)
               (display #$(juniper-command host '("cli" "show" "route" "advertising-protocol" "bgp" "85.235.192.226"))
                        port)
               (display #$(juniper-command host '("cli" "show" "route" "advertising-protocol" "bgp" "85.235.198.236"))
                        port))))))))
