...

{
  ...
  networking.firewall =
    let
      firewallRulesFile = builtins.toFile "iptables-save.txt" ''
        # Generated by iptables-save v1.8.11 (nf_tables) on Mon Jan 19 22:51:25 2026
        *mangle
        :PREROUTING ACCEPT [136533:204410648]
        :INPUT ACCEPT [224439:211057920]
        :FORWARD ACCEPT [7978:631986]
        :OUTPUT ACCEPT [231009:211289597]
        :POSTROUTING ACCEPT [238987:211921583]
        :output-hev - [0:0]
        :prerouting-hev - [0:0]
        -A PREROUTING -p tcp -m tcp --dport 80 -j prerouting-hev
        -A PREROUTING -p udp -m udp --dport 80 -j prerouting-hev
        -A PREROUTING -p tcp -m tcp --dport 443 -j prerouting-hev
        -A PREROUTING -p udp -m udp --dport 443 -j prerouting-hev
        -A OUTPUT -p tcp -m tcp --dport 80 -j output-hev
        -A OUTPUT -p udp -m udp --dport 80 -j output-hev
        -A OUTPUT -p tcp -m tcp --dport 443 -j output-hev
        -A OUTPUT -p udp -m udp --dport 443 -j output-hev
        -A output-hev -m mark --mark 0x438 -j RETURN
        -A output-hev -m set --match-set byp4 dst -j RETURN
        -A output-hev -p tcp -j MARK --set-xmark 0x440/0xffffffff
        -A output-hev -p udp -j MARK --set-xmark 0x440/0xffffffff
        -A prerouting-hev -m mark --mark 0x438 -j RETURN
        -A prerouting-hev -m set --match-set byp4 dst -j RETURN
        -A prerouting-hev -p tcp -j TPROXY --on-port 1088 --on-ip 0.0.0.0 --tproxy-mark 0x440/0xffffffff
        -A prerouting-hev -p udp -j TPROXY --on-port 1088 --on-ip 0.0.0.0 --tproxy-mark 0x440/0xffffffff
        COMMIT
        # Completed on Mon Jan 19 22:51:25 2026
        # Generated by iptables-save v1.8.11 (nf_tables) on Mon Jan 19 22:51:25 2026
        *filter
        :INPUT ACCEPT [224439:211057920]
        :FORWARD ACCEPT [7978:631986]
